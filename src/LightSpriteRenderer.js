...
const { ObjectRenderer, WebGLRenderer, Texture } = PIXI
...
export default class LightSpriteRenderer extends ObjectRenderer {
  ...
  render(sprite) {
    ...
    const vertexData = sprite.computedGeometry
      ? sprite.computedGeometry.vertices
      : sprite.vertexData
    const diffuseTexture = sprite.diffuseTexture
      ? sprite.diffuseTexture
      : sprite._texture
    const normalTexture = sprite.normalTexture
      ? sprite.normalTexture
      : LightSpriteRenderer.defaultNormalTexture
  ...
  static __renderWebGL_Sprite(renderer) {
    const sprite = this
